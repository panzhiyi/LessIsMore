...
class S3DIS(Dataset):
    ...
    def __init__(self,
                 ...
                 use_val_mask: bool = False,
                 num_pos_labeled: int = 16
                 ):
        ...
        self.use_val_mask = use_val_mask
        self.num_pos_labeled = num_pos_labeled
        ...
        processed_root = os.path.join('/userhome/LiM', 'processed')
    ...
    def __getitem__(self, idx):
        ...
        if self.presample:
            ...
            mask_path = os.path.join(
                self.mask_root, self.data_list[data_idx] + '.npy')
            if False:# os.path.exists(mask_path) and self.use_val_mask:
                mask = np.load(mask_path).astype(np.int32)
                num_label = mask.shape[0]
                pos_label = coord[mask,:]
            else:
                num_label = 0
        else:
            ...
            if self.split == 'train':
                label_tmp = np.ones_like(label) * self.ignore_index
                label_tmp[mask] = label[mask]
                label = label_tmp
            num_label = mask.shape[0]
            pos_label = coord[mask,:]
            
            mask = (label != self.ignore_index).squeeze(-1)
            coord, feat, label = crop_pc(
                coord, feat, label, self.split, self.voxel_size, self.voxel_max,
                downsample=not self.presample, variable=self.variable, shuffle=self.shuffle, mask=mask, label_aware=self.label_aware)
            # TODO: do we need to -np.min in cropped data?
        label = label.squeeze(-1).astype(np.long)
        if num_label != 0:
            coord_label = np.concatenate((coord,pos_label), axis=0)
        else:
            coord_label = coord
        data = {'pos': coord_label, 'x': feat, 'y': label}
        # pre-process.
        if self.transform is not None:
            data = self.transform(data)
        if num_label != 0:
            pos_labeled = data['pos'][-num_label:]
            data['pos'] = data['pos'][:-num_label] 
        else:
            pos_labeled = []
        if not self.use_val_mask:
            num_label = 0
        if num_label >= self.num_pos_labeled:
            idx_pos_labeled = np.random.choice(num_label, self.num_pos_labeled, False)
            data['pos_labeled'] = pos_labeled[idx_pos_labeled,:]
        else:
            idx_random = np.random.choice(coord.shape[0], self.num_pos_labeled - num_label, False)
            if num_label!=0:
                data['pos_labeled'] = torch.vstack([pos_labeled, data['pos'][idx_random]])
            else:
                data['pos_labeled'] = data['pos'][idx_random]
