...
from ..data_util import crop_pc, voxelize, rgb2hsv, rgb2lab_matrix
...
class S3DIS(Dataset):
    ...
    def __init__(self,
        ...
        processed_root = os.path.join('/userhome/LiM/upsampling/', 'processed')
    ...
    def __getitem__(self, idx):
        ...
        downsampling_idx = np.random.choice(coord.shape[0], int(coord.shape[0]/8))
        coord = coord[downsampling_idx]
        data['y'] = data['pos']
        data['pos'] = data['pos'][downsampling_idx]
        data['x'] = data['x'][downsampling_idx]
        
        if 'heights' not in data.keys():
            data['heights'] =  torch.from_numpy(coord[:, self.gravity_dim:self.gravity_dim+1].astype(np.float32))
        else:
            data['heights'] = data['heights'][downsampling_idx]
        return data
