...
class SegHead(nn.Module):
    def __init__(self,
        ...
        self.final = create_convblock1d(mlps[-2], mlps[-1], act_args=None)
...
class UpSamplingHead(nn.Module):
    def __init__(self,
        ...
        #self.final1 = create_convblock1d(int(mlps[-2]/2), mlps[-2], act_args=act_args)
        self.final = create_convblock1d(int(mlps[-2]/2), mlps[-1], act_args=None)
    ...
    def forward(self, end_points):
        if self.global_feat is not None: 
            ...
            end_points = torch.cat((end_points, global_feats), dim=1)
        feat = self.head(end_points)
        feat_up = feat.reshape(-1,int(feat.shape[1]/2),feat.shape[2]*2)
        #feat_up = self.final1(feat_up)
        #feat_up = feat_up.reshape(-1,int(feat_up.shape[1]/2),feat_up.shape[2]*2)
        logits = self.final(feat_up)
        return logits, feat
...
class ColorHead(nn.Module):
    def __init__(self,
        ...
        self.head = nn.Sequential(*heads)
        self.final = create_convblock1d(mlps[-2], mlps[-1], act_args={'act': 'tanh'})
