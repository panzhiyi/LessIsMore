...
class GlobalSparseAttention(nn.Module):
    def __init__(self,
        ...
        self.posencoder = create_convblock2d(3, channels[0], norm_args=norm_args, act_args=None, **conv_args)
        self.att_k = create_convblock1d(channels[0], channels[-1] // self.mid_channel, norm_args=None, act_args=None)
        self.att_q = create_convblock1d(channels[0], channels[-1] // self.mid_channel, norm_args=None, act_args=None)
        self.att_v = create_convblock2d(channels[0], channels[-1], norm_args=norm_args, act_args=act_args)
        self.actfunc = create_act(act_args)
        self.att_vec = nn.Sequential(nn.BatchNorm2d(channels[-1] // self.mid_channel), self.actfunc, nn.Conv2d(channels[-1] // self.mid_channel, channels[-1] // self.mid_channel, kernel_size=1), nn.BatchNorm2d(channels[-1] // self.mid_channel), self.actfunc, nn.Conv2d(channels[-1] // self.mid_channel, channels[-1] // self.mid_channel, kernel_size=1))
        self.reduction = reduction
        self.norm1 = nn.BatchNorm1d(channels[-1])

    def forward(self, px) -> torch.Tensor:
        p, x, pos_labeled = px #p(B,N,3) x(B,C,N)
        x_ = self.actfunc(self.norm1(x))
        x_k = self.att_k(x_)
        x_q = self.att_q(x_)
        # need add grouper(p,p_labeled,x_q), group x_q(B, N, C)->(B, C, N, K)
        # (B, C, N, 1) - (B, C, 1, K) * (B, C, N, K)
        dp, x_k = self.grouper(p, pos_labeled, x_k)
        _, x_v = self.grouper(p, pos_labeled, x_)
        posenc = self.posencoder(dp)#posenc(B, 512, K)
        x_k = x_k - x_q.unsqueeze(-1)
        x_k = self.att_vec(x_k)
        x_k = nn.functional.softmax(x_k, dim=-1)
        x_v = self.att_v(x_v + posenc)
        #x_v = x_v + posenc
        #x_v = x_v.unsqueeze(-2).repeat(1,1,x.shape[-1],1)
        B, C, N, K = x_v.size()
        x_v = x_v.view(B, self.mid_channel, C // self.mid_channel, N, K) * x_k.unsqueeze(1)
        x_v = x_v.view(B, C, N, K)
        x_v = x_v.sum(-1)
        return x_v
...
class LocalAggregation(nn.Module):
    def __init__(self,
        ...
        self.grouper = create_grouper(group_args)
        posencoder = []
        posencoder.append(create_convblock2d(3, channels[0] // 2, norm_args=norm_args, act_args=act_args, **conv_args))
        posencoder.append(create_convblock2d(channels[0] // 2, channels[0], norm_args=norm_args, act_args=None, **conv_args))
        self.posencoder = nn.Sequential(*posencoder)
...
class SetAbstraction(nn.Module):
    ...
    def forward(self, px):
        ...
        if self.is_head:
            ...
            x = self.pool(self.convs(self.posencoder(dp) + xj))
            if self.use_res:
        ...
        if pos_labeled==None:
            ...
            return p, x, pos_labeled

class LocalAggregation_old(nn.Module):
...
class SetAbstraction_old(nn.Module):
...
class AttInvResMLP(nn.Module):
    def __init__(self,
                 in_channels,
                 norm_args=None,
                 act_args=None,
                 aggr_args={'feature_type': 'pj', "reduction": 'max'},
                 group_args={'NAME': 'ballquery',
                             'radius': 0.1, 'nsample': 16},
                 conv_args=None,
                 expansion=1,
        ...
        self.convs = GlobalSparseAttention([in_channels, in_channels],
                                         norm_args=norm_args, act_args=act_args, group_args=group_args,
                                         conv_args=conv_args, **aggr_args)
        if num_posconvs < 1:
            channels = []
        elif num_posconvs == 1:
...
class PointNextEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.blocktype = ['InvResMLP', 'InvResMLP', 'InvResMLP', 'InvResMLP', 'AttInvResMLP']
        self.blocks = blocks
        ...
        for i in range(len(blocks)):
            if isinstance(self.blocktype[i], str):
                block = eval(self.blocktype[i])
    ...
    def _make_enc(self, block, channels, blocks, stride, group_args, is_head=False):
        ...
        for i in range(1, blocks):
            ...
            if block == eval('AttInvResMLP'):
                group_args.NAME = 'group_labeled'
    ...
    def forward_seg_feat(self, p0, f0=None):
        if hasattr(p0, 'keys'):
            p0, f0, pos_labeled  = p0['pos'], p0.get('x', None), p0['pos_labeled']
            #add pos_labeled
        if f0 is None:
            f0 = p0.clone().transpose(1, 2).contiguous()
        p, f = [p0], [f0]
        for i in range(0, len(self.encoder)):
            if i==len(self.encoder)-1:
                _p, _f, _ = self.encoder[i]([p[-1], f[-1], pos_labeled])#add pos_labeled
            else:
