...
class QueryAndGroup(nn.Module):
    def __init__(self, radius: float, nsample: int,
        ...
        self.return_with_idx = return_with_idx
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[
        ...
        if features is not None:
            ...
            if self.return_with_idx:
                return grouped_xyz, grouped_features, idx
            return grouped_xyz, grouped_features
        else:
            if self.return_with_idx:
                return grouped_xyz, None, idx
...
class GroupLabeled(nn.Module):
    def __init__(self, ):
        super().__init__()

    def forward(self, new_xyz: torch.Tensor, xyz: torch.Tensor, features: torch.Tensor = None):
        """
        :param xyz: (B, K, 3) xyz coordinates of the labeled points
        :param new_xyz: (B, N, 3) xyz coordinates of the all points
        :param features: (B, C, N) descriptors of the features
        :return:
            new_features: (B, C, N, K)
        """

        B, N, _ = new_xyz.shape
        dist = torch.cdist(xyz, new_xyz)
        idx = torch.argmin(dist, dim=-1).contiguous().int().cuda(non_blocking=True)#(B,K)
        idx = torch.repeat_interleave(idx.unsqueeze(1), N, dim=1)

        #idx_center = torch.range(0,N-1).repeat(B).view(B,N,1).contiguous().int().cuda(non_blocking=True)
        #idx = torch.concat((torch.repeat_interleave(idx.unsqueeze(1), N, dim=1),idx_center),dim=2)
        xyz_trans = new_xyz.transpose(1, 2).contiguous()
        grouped_xyz = grouping_operation(xyz_trans, idx)
        grouped_xyz = grouped_xyz - xyz_trans.unsqueeze(-1)
        grouped_features = grouping_operation(features, idx)
...
def create_grouper(group_args):
    ...
    elif method == 'group_labeled':
        grouper = GroupLabeled()
    else:
        grouper = GroupAll()
    return grouper
