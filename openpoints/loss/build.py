...
class HardestContrastLoss(torch.nn.Module):
    def __init__(self, weight, dimension, dimension_sampled, m_p = 0.1, m_n = 1.4):
        super(HardestContrastLoss, self).__init__()
        self.LARGE_NUM = 1e9
        self.dimension = dimension
        self.dimension_sampled = dimension_sampled
        self.m_p = 0.1
        self.m_n = 1.4
        # self.mask = F.one_hot(torch.arange(self.dimension))
    
    def forward(self, f1, f2):
        f1 = F.normalize(f1, p=2, dim=1)
        f2 = F.normalize(f2, p=2, dim=1)
        positive_distance = torch.norm(f1 - f2, dim=1)
        idx_sample1 = np.random.choice(self.dimension, self.dimension_sampled)
        idx_sample2 = np.random.choice(self.dimension, self.dimension_sampled)
        f1_sampled = f1[:,:,idx_sample1]
        f2_sampled = f2[:,:,idx_sample2]
        negative1 = torch.norm(f1.unsqueeze(-1) - f2_sampled.unsqueeze(-2), dim=1)
        negative1, _ = torch.min(negative1,2)
        negative2 = torch.norm(f2.unsqueeze(-1) - f1_sampled.unsqueeze(-2), dim=1)
        negative2, _ = torch.min(negative2,2)
        loss = torch.mean(F.relu(positive_distance - self.m_p).pow(2)) + 0.5 * torch.mean(F.relu(self.m_n - negative1).pow(2) + F.relu(self.m_n - negative2).pow(2))
        return loss

    
@LOSS.register_module()
class ContrastLoss(torch.nn.Module):
    def __init__(self, weight, dimension, dimension_sampled, temperature = 1):
    ...
    def forward(self, f1, f2):
        ...
        idx_sample2 = np.random.choice(self.dimension, self.dimension_sampled)
        f2_sampled = f2[:,:,idx_sample2]
        similar = torch.bmm(torch.transpose(f1, 1, 2), f2_sampled) / self.temperature

        positive_similar = torch.sum(f1 * f2, dim = 1)
        negative_similar = torch.mean(torch.exp(similar),dim=-1)
        loss = -1.0 * torch.mean(positive_similar - torch.log(negative_similar))
...
'''    
    def forward(self, f1, f2):
        f1 = F.normalize(f1, p=2, dim=1)
        f2 = F.normalize(f2, p=2, dim=1)
        similar = torch.bmm(torch.transpose(f1, 1, 2), f2) / self.temperature
        
        positive_similar = torch.diagonal(similar, dim1 = 1, dim2 = 2)
        negative_similar = (torch.sum(torch.exp(similar),dim=1) - torch.exp(positive_similar)) / (self.dimension - 1)
        loss = -1.0 * torch.mean(positive_similar - torch.log(negative_similar))
...
class CDLoss(torch.nn.Module):
    ...
    def forward(self, sample_trg, sample_src):
...
class LABLoss(torch.nn.Module):
    def __init__(self, weight = None, HL_mode = 0):
        super(LABLoss, self).__init__()
        self.HL_mode = HL_mode
        self.MSE = MSELoss(reduction = 'none')
    
    def forward(self, pred, ab, L = None):
        loss = self.MSE(pred, ab)
        if self.HL_mode == 0:
            weight = 1
        elif self.HL_mode == 1:
            weight = L
...
class HueLoss(torch.nn.Module):
    ...
    def forward(self, pred, H, SV = None):
        delta = torch.abs(H - pred)
        S, V = SV[:,0:1], SV[:,1:2]
        loss = torch.square(0.5 - torch.abs(delta-0.5))
        if self.HL_mode == 0:
            weight = 1
        elif self.HL_mode == 1:
            weight = S * V
        else:
            weight = torch.sqrt(S) * torch.square(V)
        return torch.mean(weight * loss)
